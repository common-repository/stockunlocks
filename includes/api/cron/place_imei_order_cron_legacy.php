...
function suwp_dhru_place_imei_order_cron( $post_id ) {
    ...
    // NOTE: Not really using the $post_id above, bec need to go through all orders that haven't yet been processed
    // This $post_id's orders, if any, will already be queued up to be processed with the others.
    ...
    foreach( $suwp_orders as $order ):
        ...
        $suwp_order_items = $wpdb->get_results("select order_item_id, order_id from ".$wpdb->prefix."woocommerce_order_items where order_id=". $current_order_id . " ORDER BY order_id ASC" );
        
        error_log( '>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> TOTAL NUMBER OF ORDER ITEMS FOR ORDER #' . $current_order_id . ' = ' .  count( $suwp_order_items ) );
        ...
        foreach( $suwp_order_items as $key => $loop_order_item_id ):
            ...
            error_log( '' );
            error_log( '' );
            error_log( '>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>. _product_id = ' .  $product_id );
            error_log( '>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>. Product Title = ' .  get_the_title( $product_id ) );
            error_log( '>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>. _suwp_api_provider = ' .  $post_id );
            
            // initialize the variables to be used later
            $suwp_dhru_serviceid = 0;
            $qty_sold = 0;
            $suwp_qty_sent = 0;
            $suwp_brand_id = 0;
            $suwp_model_id = 0;
            ...
            if ( has_term( 'Remote Service', 'product_cat', $product_id ) ) {
                
                error_log( '>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> REMOTE SERVICE' );
                ...
                // ALSO, make sure that it's published so as not to crash when attempting to include the file below
                $suwp_temp_post = get_post( $post_id );
                
                if ( is_object ( $suwp_temp_post ) ) {
                            
                    $suwp_temp_title = get_the_title( $post_id );
                    $suwp_temp_status = get_post_status( $post_id );
                    
                    error_log( '>>>>>>>>>>>>>> Cron: ATTEMPT TO PLACE IMEI ORDER, for post_id = ' . $post_id . ', post_status = ' . $suwp_temp_status . ', provider = ' . $suwp_temp_title . ' <<<<<<<<<<<<<<');
                    ...
                    if ( $suwp_temp_status != 'publish' ) {
                        ...
                        error_log( '>>>>>>>>>>>>>> POST IS Active AND PUBLISHED. PROCESSING ... <<<<<<<<<<<<<<');
                    }
            
                } else {
                
                    error_log( '>>>>>>>>>>>>>> NON-EXISTING UNLOCKING PROVIDER WITH ID: ' . $post_id . ' FOR THIS PRODUCT FOR ORDER #' . $current_order_id . '. NOT PROCESSING.' );
                    $suwp_activeflag = 0;
                }
                ...
                error_log( '>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> NOT A REMOTE SERVICE, NOT PROCESSING.' );
                $suwp_activeflag = 0;
            }
